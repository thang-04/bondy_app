...
void main() {
    ...
    final apiClient = await authenticatedClient(
      MockClient((request) async {
        ...
        switch (request.url.path) {
          ...
          default:
            fail(
              'Unexpected authenticated profile request: ${request.method} ${request.url}',
            );
        }
      }),
    );
  ...
  test('discover service calls discover and swipe endpoints', () async {
    ...
    expect(
      seen,
      containsAll(['GET /api/discover/profiles', 'POST /api/swipes']),
    );
  });
  ...
  test('chat service calls chat list and message endpoints', () async {
    ...
    final apiClient = await authenticatedClient(
      MockClient((request) async {
        ...
        if (request.url.path == '/api/chats/chat-1/messages' &&
            request.method == 'GET') {
          return http.Response(jsonEncode({'success': true, 'data': []}), 200);
        }
        if (request.url.path == '/api/chats/chat-1/messages' &&
  ...
  test('home service calls authenticated home content endpoint', () async {
    final apiClient = await authenticatedClient(
      MockClient((request) async {
        expect(request.method, 'GET');
    ...
    final result = await service.fetchHomeContent();

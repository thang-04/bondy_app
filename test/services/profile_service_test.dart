...
void main() {
  ...
  test('updates authenticated profile setup fields', () async {
    final apiClient = await authenticatedClient(MockClient((request) async {
      ...
      expect(request.url.toString(), 'https://api.example.com/api/profile/me');
  ...
  test('updates authenticated location', () async {
    final apiClient = await authenticatedClient(MockClient((request) async {
      expect(request.method, 'PATCH');
      expect(request.url.toString(), 'https://api.example.com/api/profile/location');
      expect(request.headers['authorization'], 'Bearer access-token');
      expect(jsonDecode(request.body), {
        'city': 'Ho Chi Minh',
        'latitude': 10.762622,
  ...
  test('surfaces profile validation failures', () async {
    ...
    await expectLater(
      service.updateProfile(fullName: '', gender: 'invalid', birthDate: 'bad-date'),
  ...
  test('saves selected interests with auth', () async {
    final apiClient = await authenticatedClient(MockClient((request) async {
      ...
      expect(request.url.toString(), 'https://api.example.com/api/profile/interests');
      expect(request.headers['authorization'], 'Bearer access-token');
